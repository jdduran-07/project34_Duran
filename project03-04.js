...
//define star images function
function starImages(rating) {
      let imageText = ''; //declare imageText

      //Loop through ratings and increase counter 
      for (let i = 1; i <= rating; i++)
            imageText += "<img src= 'star.png' alt = ''>"
      

      return imageText; //return corresponding result
}

function getReview() {
      // Loop through reviews get reviewCode

    for (let i = 0; i < reviewers.length; i++) { //loop to add stars by rating
    let reviewCode = ''; //declare reviewCode
      
      
 
         // Set the class for the table based on the review type
         if (reviewType[i] === 'P') {
          reviewCode += "<table class='prime'>"; //prime review type
        } else if (reviewType[i] === 'N') {
          reviewCode += "<table class='new'>"; //new review type
        } else {
          reviewCode += "<table>"; //default table
        }
        
        //Rating with # of stars
      ...
      reviewCode += "</table>";
       ...
       // Insert the review code into the first <article> element
